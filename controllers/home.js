...
module.exports = {
    ...
    category: async (req, res) => {
        ...
        try {
            ...
            // res.render('client/category', { data: post, profileUser: InfoUser});
            ...
            if(!token)
            {
                await Post.find({}).sort(SortTime).limit(12).exec(function (err, docs) {
                    if (err) {
                        ...
                        res.render('client/category', { status: ["", "", "Lỗi server"] });
                    }
                    else {
                        InfoUser = null;
                        data_product = docs;
                        res.render('client/category', { title: 'Express', data: docs, profileUser: InfoUser });
